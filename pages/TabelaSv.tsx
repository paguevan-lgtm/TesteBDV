...
export default function TabelaSv({ theme, tableTab, setTableTab, currentOpDate, getTodayDate, analysisDate, setAnalysisDate, analysisRotatedList, editName, tempName, tempVaga, setEditName, setTempName, setTempVaga, saveDriverName, spList, setSpList, notify, dbOp, systemContext, updateSvDriver, handleSvBaixar, handleSvRiscar, triggerUndo, ganchos, effectiveFolgas, getFolgasForDate, user, rotationBaseDate, cannedMessages, addCannedMessage, updateCannedMessage, deleteCannedMessage }: any) {
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto w-full min-h-[70vh]">
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                    <div className={`${theme.card} border ${theme.border} p-6 rounded-2xl max-w-sm w-full shadow-2xl anim-scale`}>
                        ...
                        <p className="text-center opacity-60 text-sm mb-6">
                            Isso removerá todos os motoristas da tabela atual ({tableTab === 'sv6' ? '6:00' : '18:00'}). Esta ação não pode ser desfeita.
                        </p>
            ...
            <div id="table-tabs" className="flex p-1 bg-black/20 rounded-xl border border-white/5 gap-1">
                <button onClick={()=>setTableTab('sv6')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tableTab==='sv6' ? theme.primary : 'hover:bg-white/5 opacity-60'}`}>Tabela 6:00</button>
                <button onClick={()=>setTableTab('sv18')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tableTab==='sv18' ? theme.primary : 'hover:bg-white/5 opacity-60'}`}>Tabela 18:00</button>
                <button onClick={()=>setTableTab('mensagens')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tableTab==='mensagens' ? theme.primary : 'hover:bg-white/5 opacity-60'}`}>Mensagens</button>
            </div>
            
            {(tableTab === 'sv6' || tableTab === 'sv18') && (
                <div className={`${theme.card} p-5 rounded-xl border ${theme.border} anim-fade`}>
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        ...
                        <h3 className="text-lg font-bold opacity-80">
                            {tableTab === 'sv6' ? 'Tabela 6:00' : 'Tabela 18:00'}
                        </h3>
                        <div className="flex items-center gap-2 bg-black/30 p-1 rounded-lg">
                            ...
                            <button onClick={() => onPrint('print-tabela-list', 'Tabela_SV', tableTab === 'sv6' ? 'TABELA 6:00' : 'TABELA 18:00', { forceCols: 2, date: analysisDate })} className="ml-4 p-2 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30 transition-colors" title="Salvar como Imagem (2 Colunas)"><Icons.Print size={20}/></button>
                        </div>
                    </div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {!isBlocked && (
                                                        <div className={`flex items-center gap-1 ${driver.riscado ? 'opacity-30 pointer-events-none' : ''}`}>
                                                            {!driver.baixou ? (
                                                                <>
                                                                    <input className="w-14 bg-black/40 border border-white/10 rounded px-1 py-1 text-xs font-mono text-center outline-none focus:border-white/30 hide-on-print" value={driver.time1 || ''} onChange={(e) => updateSvDriver(driver.id, { time1: e.target.value })} placeholder="00:00" />
                                                                    <input className="w-14 bg-black/40 border border-white/10 rounded px-1 py-1 text-xs font-mono text-center outline-none focus:border-white/30 hide-on-print" value={driver.time2 || ''} onChange={(e) => updateSvDriver(driver.id, { time2: e.target.value })} placeholder="00:00" />
                                                                    <input className="w-8 bg-black/40 border border-white/10 rounded px-1 py-1 text-xs font-mono text-center outline-none focus:border-white/30 hide-on-print" value={driver.num || ''} onChange={(e) => updateSvDriver(driver.id, { num: e.target.value })} placeholder="Nº" />
                                                    ...
                                                    {!isBlocked && (
                                                        <button onClick={() => handleSvRiscar(driver.id)} className={`p-1.5 rounded-lg border transition-all hide-on-print ${driver.riscado ? 'bg-red-500 text-white border-red-500' : 'bg-white/5 text-white/40 border-white/10 hover:bg-white/10'}`} title="Riscar"><Icons.Slash size={14}/></button>
                                                    )}
                                                    <button onClick={() => handleSvBaixar(driver.id)} className={`p-1.5 border rounded-lg transition-all hide-on-print ${driver.baixou ? 'bg-orange-500 text-white border-orange-500' : 'bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20'}`} title={driver.baixou ? "Cancelar Baixar" : "Baixar"}>{driver.baixou ? <Icons.X size={14}/> : <Icons.ArrowDown size={14}/>}</button>
                                                </div>
                                            </div>
                                        </SortableRow>
                                    );
